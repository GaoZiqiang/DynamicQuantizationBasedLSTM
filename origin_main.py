...
class Dictionary(object):
    ...
    def add_word(self, word):
        if word not in self.word2idx:# 可以实现去重
...
class Corpus(object):
    def __init__(self, path):
        ...
        # self.valid = self.tokenize(os.path.join(path, 'valid.txt'))# 暂时不同valid集
...
### 简单测试
print("====== 对pre-trained model做简单测试 ======")
input_ = torch.randint(ntokens, (1, 1), dtype=torch.long)# 输入
